...
# ---------------------------------------------------------------------------
# Module-level async helpers — called via asyncio.run() at 4-space indentation
# in standalone test functions (not class methods), keeping QG check clean.
# ---------------------------------------------------------------------------


async def _asyncpg_ping(dsn: str) -> str:
    """Connect to a PostgreSQL DB and return str(SELECT 1) result."""
    import asyncpg  # guard: only imported when asyncpg is available
...
async def _asyncpg_list_tables(dsn: str) -> list[str]:
    """Return public table names from a PostgreSQL DB."""
    import asyncpg
...
async def _asyncpg_read_write_probe(dsn: str, probe_id: str) -> str:
    """Write and read a probe row; return the read-back id."""
    import asyncpg
...
async def _asyncpg_ping_void(dsn: str) -> None:
    """Connect and execute SELECT 1 (no return value)."""
    import asyncpg
...
@pytest.mark.smoke
def test_db_url_resolvable(gcp_project_id: str) -> None:
...
def test_cloud_sql_connect_and_ping(gcp_project_id: str) -> None:
    """Connect to Cloud SQL and execute SELECT 1."""
    url = _get_db_url(gcp_project_id)
    if url is None:
        pytest.skip("No DATABASE_URL available")
    dsn = url.replace("postgresql+asyncpg://", "postgresql://")
    result = asyncio.run(_asyncpg_ping(dsn))
    assert result == "1", f"SELECT 1 returned {result!r}"


@pytest.mark.smoke
@pytest.mark.skipif(not _has_asyncpg(), reason="asyncpg not installed")
def test_cloud_sql_order_state_table_accessible(gcp_project_id: str) -> None:
    """order_states table must exist (created by execution-service on startup)."""
    url = _get_db_url(gcp_project_id)
    if url is None:
        pytest.skip("No DATABASE_URL available")
    dsn = url.replace("postgresql+asyncpg://", "postgresql://")
    tables = asyncio.run(_asyncpg_list_tables(dsn))
...
@pytest.mark.smoke
@pytest.mark.skipif(not _has_asyncpg(), reason="asyncpg not installed")
def test_cloud_sql_read_write_probe(gcp_project_id: str) -> None:
    ...
    dsn = url.replace("postgresql+asyncpg://", "postgresql://")
    result = asyncio.run(_asyncpg_read_write_probe(dsn, probe_id))
...
def test_aws_rds_blocked() -> None:
    ...
    asyncio.run(_asyncpg_ping_void(rds_url))
