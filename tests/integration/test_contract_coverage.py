...
Coverage: all 171 symbols in unified_api_contracts.internal.__all__ + all
...
def _get_uic_all() -> list[str]:
    ...
    pkg_init = Path(unified_api_contracts.internal.__file__).parent / "__init__.py"
...
def test_uic_symbol_importable(symbol: str) -> None:
    ...
    assert obj is not None, (
        f"unified_api_contracts.internal.{symbol} is listed in __all__ but is not "
...
def test_uic_submodule_importable(submodule: str) -> None:
    """unified_api_contracts.internal.<submodule> must import without errors."""
    m = importlib.import_module(f"unified_api_contracts.internal.{submodule}")
